...
void calculate_score(const char *directory){
    DIR* d = NULL;
    struct dirent* file;
    char path_dir[pathSIZE], path_tr[pathSIZE];
    ...
    while ((file = readdir(d)) != NULL) {
        ...
        snprintf(path_dir, sizeof(path_dir), "%s/%s", directory, file->d_name);
        if (lstat(path_dir, &st) == -1)
            continue;

        if (S_ISDIR(st.st_mode)) {
            printf("Hunt name: %s --", file->d_name);
            calculate_score(path_dir);
        } else if(S_ISREG(st.st_mode)){
...
void handle_signals(int signal) {
    ...
    if (strcmp(p, "list_hunts") == 0) {
        ...
        else write(1, "Hunt ID doesn't exist\n", strlen("Hunt ID doesn't exist\n"));
    }
}
...
void handle_signal_stopMonitor(int signal) {
    int status;
    int pidd = waitpid(monitor_pid, &status, 0);
    if (pidd == -1) {
        perror("some error with the signals");
        exit(1);
    }
